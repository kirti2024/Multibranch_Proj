	...
	DOCKER_PASSWORD="dtoken"
		...
		stages{
			stage('Checkout'){
				steps{
					script{
   checkout scmGit(branches: [[name: '*/multbranch2']], extensions: [], userRemoteConfigs: [[url:'https://github.com/kirti2024/Multibranch_Proj.git']])
}
				}
			}
			stage('build image'){
...
stage('Deploy to K8s'){
              steps{
		script{ 
			sh """ helm helmrelease2 ./nginx-helm """
